...
// Méthodes pour gérer le like et le dislike
const likeComment = async (commentId: string) => {
  if (userVotes.value[commentId]) return; // Empêcher un nouvel vote si déjà fait
  ...
  try {
    ...
    // Stocker que l'utilisateur a voté "like" pour ce commentaire
    userVotes.value[commentId] = 'like';
    localStorage.setItem('userVotes', JSON.stringify(userVotes.value)); // Sauvegarder dans le localStorage
  } catch (err) {
...
const dislikeComment = async (commentId: string) => {
  ...
  try {
    ...
    // Stocker que l'utilisateur a voté "dislike" pour ce commentaire
    userVotes.value[commentId] = 'dislike';
    localStorage.setItem('userVotes', JSON.stringify(userVotes.value)); // Sauvegarder dans le localStorage
  } catch (err) {
    console.error('Erreur lors du dislike du commentaire:', err);
  }
};
...
<h3>Avis</h3>
    ...
    <button @click="likeComment(comment._id)" :disabled="userVotes[comment._id] === 'like'" class="vote-button">
